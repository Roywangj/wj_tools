...
### 10. [ohmyzsh.md](./ohmyzsh.md) — Shell 美化（Oh My Zsh + Powerlevel10k）

把服务器 Shell 配成 Oh My Zsh + Powerlevel10k 主题环境：安装 Zsh / Oh My Zsh、配置 语法高亮与自动补全插件。

> 更完整的「Zsh + Starship + modern CLI 工具」现代终端方案（含跨平台字体、Ghostty SSH 排错等）见 macOS 目录下的 [ubuntu-shell-config.md](../macos-setup-guide/ubuntu-shell-config.md)。
